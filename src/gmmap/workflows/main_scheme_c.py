...
CELLTYPE_KEY = "disease_Level1"
PREFIX = "Meta"
OUT_DIR = "Meta_SchemeC_AUROC_UpDownNet_outputs"

# sensitivity version 1: gentle variance penalty
VARIANCE_ALPHA = 0.5
NOISE_EPS = 1e-8
NOISE_FLOOR_Q = 0.01
WEIGHT_CAP_Q = 0.995
...
# "S2_geneSetZ","S4_geneSetZ_cellZ_geneSetZ"
# methods to run
METHODS_TO_RUN = ["wAUCell"]  #"scMRS", # or ["wAUCell"] only
...
# =========================================================
# 1) gene noise / variability penalty
# =========================================================
def compute_gene_noise(adata):
    """
    Sensitivity version 1: gentle expression-variability penalty.

    Original pipeline:
        sigma_g = sqrt(Var_c(X_cg))
        W_g = Z_g / sigma_g

    This version:
        penalty_g = (sigma_g + NOISE_EPS) ** VARIANCE_ALPHA
        W_g = Z_g / penalty_g

    Recommended for sensitivity testing:
        VARIANCE_ALPHA = 0.5

    Note:
        This is expression-variability adjustment, not strict technical-noise removal.
    """
    X = adata.X
    ...
    penalty = np.power(sigma + NOISE_EPS, VARIANCE_ALPHA)

    # Avoid extremely small penalty causing very large weights.
    valid = np.isfinite(penalty) & (penalty > 0)
    if valid.sum() > 0:
        floor = np.nanquantile(penalty[valid], NOISE_FLOOR_Q)
        if np.isfinite(floor) and floor > 0:
            penalty[valid] = np.maximum(penalty[valid], floor)

    penalty[~np.isfinite(penalty)] = np.nan
    penalty[penalty <= 0] = np.nan
    return dict(zip(adata.var_names, penalty))
...
# =========================================================
# 3) weights: w = z / penalty (positive only)
# =========================================================
def compute_weights_from_z(gene_z_series, gene_noise, adata):
    ...
    penalty = np.array([gene_noise[g] for g in genes], dtype=float)

    mask = np.isfinite(z) & np.isfinite(penalty) & (penalty > 0) & (z > 0)
    genes = [g for g, m in zip(genes, mask) if m]
    if len(genes) == 0:
        return None, None

    w = z[mask] / penalty[mask]

    # Optional robustness cap for extreme weights.
    if WEIGHT_CAP_Q is not None:
        valid_w = np.isfinite(w) & (w > 0)
        if valid_w.sum() > 20:
            cap = np.nanquantile(w[valid_w], WEIGHT_CAP_Q)
...
def compute_relation_auroc_updown_delta_netcells(
    ...
    if N <= 1:
        mats = {
            "AUROC_up": pd.DataFrame(au_up, index=ct_levels, columns=traits),
            "AUROC_down": pd.DataFrame(au_dn, index=ct_levels, columns=traits),
            "AUROC_effect_up": pd.DataFrame(ef_up, index=ct_levels, columns=traits),
            "AUROC_effect_down": pd.DataFrame(ef_dn, index=ct_levels, columns=traits),
            "AUROC_effect_net_DELTA": pd.DataFrame(ef_delta, index=ct_levels, columns=traits),
            "AUROC_netCell": pd.DataFrame(au_netcell, index=ct_levels, columns=traits),
            "AUROC_effect_netCell": pd.DataFrame(ef_netcell, index=ct_levels, columns=traits),
            "final_AUROC": pd.DataFrame(au_final, index=ct_levels, columns=traits),
            "final_effect": pd.DataFrame(ef_final, index=ct_levels, columns=traits),
        }
    ...
    mats = {
        ...
        # optional p matrices
        "p_up": pd.DataFrame(p_up_mat, index=ct_levels, columns=traits),
        "p_down": pd.DataFrame(p_dn_mat, index=ct_levels, columns=traits),
        "p_delta": pd.DataFrame(p_delta, index=ct_levels, columns=traits),
        "p_netCell": pd.DataFrame(p_netcell, index=ct_levels, columns=traits),
        "final_p": pd.DataFrame(p_final, index=ct_levels, columns=traits),
    }
...
def process_stage_auroc_all_and_thresholds(
    ...
    mats["AUROC_effect_net_DELTA"].to_csv(m_del_path, compression="gzip")
    mats["AUROC_netCell"].to_csv(m_netC_auc_path, compression="gzip")
    mats["AUROC_effect_netCell"].to_csv(m_netC_eff_path, compression="gzip")
    ...
    export_topk_summaries(
        effect_df=mats["AUROC_effect_net_DELTA"],
        stats_long=stats_df,
    ...
    export_topk_summaries(
        effect_df=mats["AUROC_effect_netCell"],
        stats_long=stats_df,
